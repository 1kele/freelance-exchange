from src.exceptions import (
    ...
    SelfReviewForbiddenException,
    ReviewAlreadyExistsException,
    InvalidReviewParticipantsException,
...
class ReviewService(BaseService):
    async def create_review(self, data: ReviewAdd, user_id: int) -> None:
        ...
        if (
            full_info.freelancer_id != data.target_id
            and full_info.customer_id != data.target_id
        ):
        ...
        existing = await self.db.review.get_filter_by(
            order_id=data.order_id, author_id=user_id
        )
        if existing:
        ...
        new_rating = round(
            (current_user_data.rating * (count - 1) + data.rating) / count, 2
        )
    ...
    async def get_all_reviews(self, user_id: int) -> list[ShowReview]:
        try:
        ...
        formated_reviews = [
            ShowReview.model_validate(r, from_attributes=True) for r in reviews
        ]
