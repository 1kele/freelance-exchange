...
class AuthenticationService(BaseService):
    async def register(
        ...
        new_data = UserAdd(
            hashed_password=hashed_password, **data.model_dump(exclude={"password"})
        )
        try:
    ...
    async def login(
        ...
        if not Authentication.password_hash.verify(
    ...
    async def get_me(self, current_user: CurrentUserDep):
