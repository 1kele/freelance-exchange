from src.exceptions import AlreadyRespondedException, PermissionDeniedException, AlreadyAcceptedException
from src.schemas.order import OrderAccept, OrderStatus
from src.schemas.response import ResponseAdd, ResponseAddRequest, ResponseStatusRequest, ResponseStatus, Response
...
class ResponsesService(BaseService):
    ...
    async def _change_response_status(
        ...
        await OrderService(self.db).check_order_ownership(response.order_id, current_user_id)
        await self.db.response.edit(
            ResponseStatusRequest(status=new_status),
            id=response_id
        )
    ...
    async def respond_to_order(
        ...
        freelancer = await self.db.response.get_filter_by(order_id=order_id, freelancer_id=user_id)
        if freelancer:
            raise AlreadyRespondedException
        new_data = ResponseAddRequest(order_id=order_id, freelancer_id=user_id, **data.model_dump())
    ...
    async def accept_response(
        ...
        response = await self.db.response.get_one(id=response_id)
        await self._change_response_status(response_id, current_user_id, ResponseStatus.accepted)
        ...
        await self.db.order.edit(
            ...
            deadline_date=deadline_date) ,id=response.order_id)
        await self.db.commit()
    ...
    async def reject_response(
        ...
        await self._change_response_status(response_id, current_user_id, ResponseStatus.rejected)
        await self.db.commit()
