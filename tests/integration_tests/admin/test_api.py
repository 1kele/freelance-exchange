...
async def test_assign_user_admin(auth_ac_admin):
    response = await auth_ac_admin.patch(
        "/admin/assign/4", json={"role": AllRoles.freelancer}
    )
...
async def test_get_orders(auth_ac_admin):
    response = await auth_ac_admin.get(
        "/admin/orders", params={"status": OrderStatus.in_progress.value}
    )
