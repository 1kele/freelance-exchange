...
@pytest.mark.parametrize(
    "order_id, target_id, rating, text, status_code",
    [
...
async def test_add_review_errors(
    auth_ac_customer, order_id, target_id, rating, text, status_code
):
    response = await auth_ac_customer.post(
        "/reviews",
        json={
...
async def test_get_reviews_nonexistent_user(auth_ac_customer):
    response = await auth_ac_customer.get("/reviews", params={"user_id": 999})
